...
class HumanoidCore(Star):
    def __init__(self, context: Context, config: dict = None):
        ...
        logger.info("[humanoid_core] 插件加载成功 (v2.9.0)")
    ...
    def get_schedule_provider(self, cfg: dict):
        ...
        if not target:
            if fallback:
                ...
                if prov:
                    ...
                    logger.error(f"[humanoid_core] 备用 Provider '{fallback}' 不存在")
                    return None
            else:
                # 都没配置，使用全局默认
        ...
        # 指定失败，尝试备用
        if fallback:
            logger.warning(f"[humanoid_core] 指定 Provider '{target}' 不存在，尝试备用 '{fallback}'")
            prov = _get(fallback)
            if prov:
                return prov
            else:
                logger.error(f"[humanoid_core] 备用 Provider '{fallback}' 也不存在")
                return None
        else:
            logger.error(f"[humanoid_core] 指定 Provider '{target}' 不存在且未配置备用")
            return None
    ...
    async def generate_llm_daily_schedule(self, today_str: str, cfg: dict) -> list:
        ...
        for attempt in range(3):
            try:
                provider = self.get_schedule_provider(cfg)  # 使用新方法
                if not provider:
    ...
    async def _update_mood_by_message(self, event: AstrMessageEvent, qq: str, cfg: dict):
        ...
        for attempt in range(2):
            try:
                provider = self.get_target_provider(cfg)  # 情绪分析仍然使用原有方法
                if not provider:
                    break
