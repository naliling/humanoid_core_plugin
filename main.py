...
class HumanoidCore(Star):
    ...
    def _provider_aliases(self, key, provider):
        """收集 Provider 的完整标识，用于解决配置值与运行时 key 格式差异。"""
        aliases = set()
        if key is not None:
            aliases.add(str(key))
        for attr in (
    ...
    def _resolve_provider_by_name(self, name: str, cfg: dict, purpose: str = "Provider"):
        ...
        if callable(get_provider):
            try:
                ...
                if prov:
                    logger.info(f"[humanoid_core] {purpose}精确命中: {target}")
                    return prov
            except Exception as e:
                if cfg.get("debug_mode", False):
    ...
    def get_schedule_provider(self, cfg: dict):
        ...
        allow_global = bool(cfg.get("schedule_allow_global_fallback", False))

        if target:
            prov = self._resolve_provider_by_name(target, cfg, purpose="日程首选模型")
            if prov:
                return prov
            logger.warning(f"[humanoid_core] 日程首选模型不可用: '{target}'")

            if fallback:
                prov = self._resolve_provider_by_name(fallback, cfg, purpose="日程备用模型")
                if prov:
                    logger.warning(f"[humanoid_core] 日程改用明确配置的备用模型: '{fallback}'")
                    return prov
                logger.warning(f"[humanoid_core] 日程备用模型也不可用: '{fallback}'")

            if allow_global and hasattr(self.context, "get_using_provider"):
                try:
                    prov = self.context.get_using_provider()
                    if prov:
                        logger.warning(
                            "[humanoid_core] 专用模型不可用，已按配置允许回退到全局默认 Provider"
                        )
                        return prov
                except Exception as e:
                    if cfg.get("debug_mode", False):
                        logger.debug(f"[humanoid_core] 获取全局默认 Provider 失败: {e}")
            logger.warning(
                "[humanoid_core] 已配置专用模型，但均不可用；禁止回退全局默认，改用内置日程模板"
            )
            return None
        ...
        if fallback:
            ...
            if allow_global and hasattr(self.context, "get_using_provider"):
                try:
                    ...
                    if prov:
                        logger.warning(
                            "[humanoid_core] 备用模型不可用，已按配置允许回退到全局默认 Provider"
                        )
                        return prov
                except Exception as e:
                    if cfg.get("debug_mode", False):
                        logger.debug(f"[humanoid_core] 获取全局默认 Provider 失败: {e}")
            logger.warning(
                f"[humanoid_core] 未配置首选模型且备用模型不可用: '{fallback}'；禁止使用全局默认"
            )
            return None

        # 完全未配置专用模型时保持兼容：使用 AstrBot 全局默认。
        if hasattr(self.context, "get_using_provider"):
            try:
                prov = self.context.get_using_provider()
                if prov:
                    logger.debug("[humanoid_core] 未配置日程专用模型，使用全局默认 Provider")
                    return prov
            except Exception as e:
                if cfg.get("debug_mode", False):
                    logger.debug(f"[humanoid_core] 获取全局默认 Provider 失败: {e}")
