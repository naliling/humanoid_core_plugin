...
class PromptBuilder:
    ...
    def _build_compact_behavior(
        ...
        if event_type == "conversation_started":
            ...
            event_text = "用户刚重新接上对话（约离开5～30分钟）"
        elif event_type == "user_returned":
            bucket = str(data.get("gap_bucket", ""))
            if bucket == "medium_return":
                event_text = "用户隔了约30分钟～2小时重新出现"
            elif bucket == "long_return":
                event_text = "用户隔了约2～6小时重新出现"
            else:
                event_text = "用户隔了一段时间重新出现"
        elif event_type == "long_gap":
            event_text = "用户隔了6小时以上重新出现"
        else:
            event_text = "最近出现了交流变化"

        text = (
            f"【近期情境】{event_text}。这是本轮的短期背景，参考一次即可；"
            "不要因此连续重复追问离开原因，优先自然承接当前对话。"
        )
