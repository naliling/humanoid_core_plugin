...
1. **跨天重置后，计费起点是「当前时刻」而不是当天 00:00。**
   若把起点写成午夜，当天第一次更新就会按「从午夜到现在」计费；自然恢复默认
   0.9/分钟 = 54/小时，凌晨之后的任何一条消息都会把精力直接顶到上限。
2. **自然恢复只在日程本身是休息类时段（`energy_rate` ≥ 0）时生效。**
   若无条件叠加，它的量级会完全盖住日程驱动的 ±0.1/分钟，精力曲线等于失效。
   工作、外出、社交时段严格只减不加。

`energy_natural_recovery_interval_minutes` 是自然恢复的最小计费间隔：不满一个
间隔的零头不计入。
"""
