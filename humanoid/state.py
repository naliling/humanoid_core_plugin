...
def seed_cycle_day(today: str, cycle_length: int = 28) -> int:
    """按日期确定性地播一个周期起始日，避免所有用户都从第 1 天开始。"""
...
class StateStore:
    ...
    读：直接访问 `store.data`（同步、无锁，适合只读快照）。
    写：`async with store.lock:` 内修改，然后 `store.mark_dirty()`。
    """
    ...
    async def _flush_loop(self) -> None:
        while True:
            ...
            try:
                ...
                self._warn(f"状态落盘失败，将在下次标脏后重试: {exc}")
    ...
    def flush_sync(self) -> bool:
        """同步落盘，供无事件循环的场景（如 __del__ 兜底）使用。"""
        if not self._dirty:
            return False
        payload = json.dumps(self._state, ensure_ascii=False, indent=2)
